document.addEventListener('DOMContentLoaded', () => {
  ...
  function exportSettings() {
    try {
      ...
      const exportFileDefaultName = `bocchi-image-config-${new Date().toISOString().slice(0, 10)}.json`;
      // 创建临时链接元素
      ...
      showToast('导出配置失败: ' + error.message);
    }
  }
    ...
    try {
      ...
      reader.onload = function(event) {
        try {
          ...
          // 验证导入的配置
          if (!importedConfig.appName || importedConfig.appName !== "BocchiImage" || !importedConfig.settings) {
            throw new Error('无效的配置文件格式');
          }
          
          // 提取设置
          const importedSettings = importedConfig.settings;
          
          // 验证基本字段（可选）
          if (!('allowedFormats' in importedSettings)) {
            importedSettings.allowedFormats = defaultSettings.allowedFormats;
          }
          
          // 保存到本地存储
          localStorage.setItem('bocchiImageSettings', JSON.stringify(importedSettings));
          ...
          console.error('Error parsing imported config:', parseError);
